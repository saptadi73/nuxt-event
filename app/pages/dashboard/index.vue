<template>
  <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div class="mt-3 flex flex-wrap items-end justify-between gap-5">
      <div>
        <h1 class="text-4xl font-black">Welcome to IWBIF 2026</h1>
        <p class="mt-3 text-slate-300">Your ticket, schedule, profile, networking, payment, and event updates in one place.</p>
      </div>
      <div class="glass-card rounded-2xl px-5 py-3 text-right">
        <p class="text-xs uppercase tracking-[.25em] text-slate-400">Event starts in</p>
        <p class="mt-1 text-xl font-bold text-cyan-200">{{ countdown }}</p>
      </div>
    </div>
    ...
    <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <NuxtLink v-for="item in menu" :key="item.to" :to="item.to" class="glass-card rounded-3xl p-6 transition hover:-translate-y-1 hover:border-cyan-300/40">
