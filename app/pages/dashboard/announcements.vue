<template>
  <section class="mx-auto max-w-4xl px-3 py-10 sm:px-6">
    ...
    <p v-if="pending" class="glass-card mt-8 rounded-3xl p-6 text-slate-300">{{ copy.loading }}</p>
    <p v-else-if="errorMessage" class="mt-8 rounded-2xl border border-red-300/30 bg-red-950/30 p-4 text-red-100">{{ errorMessage }}</p>
    <p v-else-if="!items.length" class="glass-card mt-8 rounded-3xl p-6 text-slate-300">{{ copy.empty }}</p>
    <div v-else class="mt-8 space-y-5">
      <article v-for="item in items" :key="item.id" class="glass-card rounded-3xl p-4 sm:p-6">
        <div class="flex flex-wrap justify-between gap-3"><span class="text-xs uppercase tracking-[.25em] text-orange-200">{{ copy.announcement }}</span><time class="text-xs text-slate-500">{{ formatDate(item.published_at || item.created_at) }}</time></div>
...
const formatDate = (value?: string | null) => value ? new Intl.DateTimeFormat(locale.value === 'zh-CN' ? 'zh-CN' : 'en-GB', { dateStyle: 'long', timeZone: 'Asia/Jakarta' }).format(new Date(value)) : '';
</script>
