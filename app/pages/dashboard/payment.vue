<template>
  <section class="mx-auto max-w-6xl px-3 py-10 sm:px-6">
    <div class="glass-card rounded-[2rem] p-4 sm:p-7">
      <p class="text-sm uppercase tracking-[.3em] text-amber-200">Payment method</p><h1 class="mt-3 text-3xl font-black sm:text-4xl">Choose how you would like to pay</h1><p class="mt-3 max-w-3xl text-sm leading-7 text-slate-300">Select manual bank transfer, direct QR Code Pay, or continue to DOKU for online payment. The final amount comes directly from your backend order.</p>
      ...
      <template v-else>
        ...
        <div v-else class="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <article class="payment-choice payment-choice-doku">
            <span class="payment-choice__tag">Online gateway</span><strong>Online Payment</strong><p>Continue securely to the hosted DOKU payment page.</p><button class="payment-choice__action disabled:cursor-not-allowed disabled:opacity-60" :disabled="submitting" @click="startPayment">{{ submitting ? 'Preparing DOKU...' : 'Continue to DOKU' }} <span aria-hidden="true">→</span></button>
          </article>
        </div>
