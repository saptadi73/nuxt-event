    ...
    <div class="glass-card rounded-[2rem] p-4 sm:p-7">
      ...
      <template v-else>
        ...
        <div v-else class="mt-6 grid gap-4 md:grid-cols-2">
          <NuxtLink :to="`/dashboard/payment-manual?order_id=${encodeURIComponent(orderId)}`" class="payment-choice payment-choice-bank" :class="{ 'payment-choice--disabled': hasPendingPayment }" :aria-disabled="hasPendingPayment" @click="preventPendingNavigation">
          ...
          <article class="payment-choice payment-choice-doku">
            <span class="payment-choice__tag">Online gateway</span><strong>Online Payment</strong><p>{{ hasPendingPayment ? 'A payment is awaiting backend confirmation. You cannot create another payment yet.' : `Continue securely to the hosted ${paymentProviderLabel} payment page.` }}</p><button class="payment-choice__action disabled:cursor-not-allowed disabled:opacity-60" :disabled="submitting || hasPendingPayment" @click="startPayment">{{ hasPendingPayment ? 'Payment pending' : submitting ? `Preparing ${paymentProviderLabel}...` : `Continue to ${paymentProviderLabel}` }} <span aria-hidden="true">→</span></button>
          </article>
        </div>
        <NuxtLink v-if="hasPendingPayment" :to="paymentStatusTo" class="mt-4 inline-flex rounded-full border border-amber-300/30 bg-amber-300/10 px-5 py-3 font-semibold text-amber-100">Check payment status</NuxtLink>
        <NuxtLink to="/dashboard/cart" class="mt-6 inline-flex rounded-full border border-white/20 px-6 py-3 text-center">Back to cart</NuxtLink>
      </template>
...
const activePayment=ref<PaymentItem|null>(null);const hasPendingPayment=computed(()=>['created','pending'].includes(activePayment.value?.transaction_status?.toLowerCase()||''));const paymentStatusTo=computed(()=>`/dashboard/payment-status?order_id=${encodeURIComponent(orderId.value)}&payment_id=${encodeURIComponent(paymentId.value)}`);const preventPendingNavigation=(event:MouseEvent)=>{if(hasPendingPayment.value)event.preventDefault();};
