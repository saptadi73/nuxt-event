<template>
  <section class="mx-auto max-w-6xl px-3 py-10 sm:px-6">
    <div class="glass-card rounded-[2rem] p-4 sm:p-7">
      <p class="text-sm uppercase tracking-[.3em] text-amber-200">Payment method</p><h1 class="mt-3 text-3xl font-black sm:text-4xl">Choose how you would like to pay</h1><p class="mt-3 max-w-3xl text-sm leading-7 text-slate-300">Choose QR Code Direct, manual bank transfer, or continue to {{ paymentProviderLabel }} for online payment. The final amount comes directly from your backend order.</p>
      ...
      <template v-else>
        ...
        <div v-else class="mt-6 grid gap-4 md:grid-cols-3">
          <NuxtLink :to="`/dashboard/payment-manual?order_id=${encodeURIComponent(orderId)}`" class="payment-choice payment-choice-bank" :class="{ 'payment-choice--disabled': paymentDisabled }" :aria-disabled="paymentDisabled" @click="preventDisabledNavigation">
            <span class="payment-choice__tag">Bank account</span><strong>Manual Bank Transfer</strong><p>View bank details, transfer the exact amount, then upload your payment proof.</p><span class="payment-choice__action">View bank details <span aria-hidden="true">→</span></span>
          </NuxtLink>
          <NuxtLink :to="`/dashboard/payment-qr?order_id=${encodeURIComponent(orderId)}`" class="payment-choice payment-choice-qr" :class="{ 'payment-choice--disabled': paymentDisabled }" :aria-disabled="paymentDisabled" @click="preventDisabledNavigation">
            <span class="payment-choice__tag">Static QRIS</span><strong>QR Code Direct</strong><p>Scan the QRIS, enter the exact package total, then upload your payment proof.</p><span class="payment-choice__action">Open QRIS payment <span aria-hidden="true">→</span></span>
          </NuxtLink>
