  ...
  <section class="mx-auto max-w-6xl px-3 py-10 sm:px-6">
    <div class="glass-card rounded-[2rem] p-4 sm:p-7">
      ...
      <template v-else-if="order">
        ...
        <OrderPaymentProgress :order="order" :attempts="paymentAttempts" @attempts-removed="ids => paymentAttempts = paymentAttempts.filter(attempt => !ids.includes(attempt.id))" />
        <button type="button" class="mt-4 rounded-full border border-white/20 px-4 py-2 text-sm disabled:opacity-50" :disabled="submitting" @click="refreshOrder">{{ locale === 'zh-CN' ? '刷新付款进度' : 'Refresh payment progress' }}</button>
        <aside v-if="showSplitNotice" class="split-notice" role="note" aria-live="polite">
          ...
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2"><p class="split-notice__eyebrow">{{ copy.paymentNotice }}</p><span class="split-notice__badge">{{ paymentPartCount }} × {{ copy.secureTransaction }}</span></div>
        ...
        <div v-else class="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <NuxtLink v-if="!paymentAttempts.length && !isPartial" :to="`${offlineRegistrationPath}?payment=offline&order_id=${encodeURIComponent(orderId)}`" class="payment-choice payment-choice-offline">
            <span class="payment-choice__tag">{{ offlineCopy.tag }}</span><strong>{{ offlineCopy.title }}</strong><p>{{ offlineCopy.description }}</p><span class="payment-choice__action">{{ offlineCopy.action }} <span aria-hidden="true">→</span></span>
          </NuxtLink>
          <article class="payment-choice payment-choice-doku" :class="{'payment-choice--split':showSplitNotice}">
            <span class="payment-choice__tag">{{ copy.gatewayTag }}</span><strong>{{ copy.onlineTitle }}</strong><p>{{ onlineDescription }}</p><button class="payment-choice__action disabled:cursor-not-allowed disabled:opacity-60" :disabled="submitting || onlinePaymentDisabled" @click="requestPayment">{{ onlineAction }} <span aria-hidden="true">→</span></button>
          </article>
        </div>
    ...
    <Teleport to="body">
      <div v-if="splitModalOpen" class="split-modal" role="dialog" aria-modal="true" aria-labelledby="split-payment-title" @click.self="splitModalOpen=false">
        <div class="split-modal__panel">
          ...
          <p class="split-notice__eyebrow">{{ copy.beforeGateway.replace('{provider}', paymentProviderLabel) }}</p>
          <h2 id="split-payment-title">{{ copy.paymentNotice }}</h2>
          <p class="mt-4 leading-7 text-slate-300">{{ copy.splitLead }}</p>
          <p class="split-modal__flow text-amber-100">{{ paymentPartCount }} × {{ copy.secureTransaction }}</p>
...
import {validateDokuCheckoutUrl} from '~/utils/dokuCheckout';
...
const offlineCopy = computed(() => locale.value === 'zh-CN' ? {
  tag: '主办方协助',
  title: '线下付款',
  description: '请先完成注册，然后通过平台外的方式付款。管理员或主办方核实收款后，将根据您的注册信息创建付款记录。',
  action: '完成注册'
} : {
  tag: 'Organizer assisted',
  title: 'Offline Payment',
  description: 'Complete your registration first, then pay outside the platform. An administrator or organizer will create the verified payment from your registration.',
...
const paymentNoticeMessages={
  en:{paymentNotice:'Payment Notice',secureTransaction:'Secure Transactions',splitLead:'Orders above IDR 9,000,000 are automatically split into payments of up to IDR 9,000,000 each. Choose your payment method on the hosted checkout page for each part.',splitStepOne:'The system will automatically guide you through each payment step.',splitStepTwo:'Your registration will be confirmed once all required transactions have been successfully completed.',splitThanks:'Thank you for your understanding.',beforeGateway:'Before you continue to {provider}',reviewPayment:'Review payment',understandContinue:'I Understand, Continue',close:'Close'},
  zh:{paymentNotice:'付款须知',secureTransaction:'笔安全交易',splitLead:'超过 9,000,000 印尼盾的订单将自动拆分，每笔付款不超过 9,000,000 印尼盾。每笔付款均在支付网关页面选择付款方式。',splitStepOne:'系统将自动引导您完成每个付款步骤。',splitStepTwo:'所有必要交易均成功完成后，您的注册才会获得确认。',splitThanks:'感谢您的理解与配合。',beforeGateway:'前往 {provider} 之前请注意',reviewPayment:'返回核对付款',understandContinue:'我已了解，继续付款',close:'关闭'}
} as const;
...
const route=useRoute();const paymentApi=usePayment();const {getEvents,getDelegatePackageCatalog}=useEvent();const registrationFlow=useRegistrationFlow();const orderId=ref(''),paymentId=ref(''),errorMessage=ref(''),requestId=ref('');const order=ref<OrderItem|null>(null);const orderItems=ref<PendingOrderProductItem[]>([]);const usdPricesByProductId=ref(new Map<string,number>());const loading=ref(true),submitting=ref(false),splitModalOpen=ref(false);const paymentProvider=computed(()=>activePayment.value?.provider?.startsWith('midtrans')?'midtrans':activePayment.value?.provider?.startsWith('doku')?'doku':paymentApi.paymentProvider);const paymentProviderLabel=computed(()=>paymentProvider.value.toUpperCase());const isPaid=computed(()=>isOrderFullyPaid(order.value));const isPartial=computed(()=>order.value?.status?.toLowerCase()==='partially_paid');const paymentPartCount=computed(()=>Math.max(order.value?.payment_sequence_count||1,order.value?.currency?.toUpperCase()==='IDR'?Math.ceil((order.value.total_amount||0)/9000000):1));const showSplitNotice=computed(()=>paymentPartCount.value>1&&!isPaid.value);const queryValue=(value:unknown)=>Array.isArray(value)?String(value[0]||''):typeof value==='string'?value:'';
const paymentAttempts=ref<PaymentItem[]>([]);const activePayment=ref<PaymentItem|null>(null);const hasPendingPayment=computed(()=>['created','pending'].includes(activePayment.value?.transaction_status?.toLowerCase()||''));const unsupportedOrderCurrency=computed(()=>Boolean(order.value&&(order.value.currency||'').toUpperCase()!=='IDR'));const paymentDisabled=computed(()=>hasPendingPayment.value||unsupportedOrderCurrency.value);const paymentStatusTo=computed(()=>`/dashboard/payment-status?order_id=${encodeURIComponent(orderId.value)}&payment_id=${encodeURIComponent(paymentId.value)}`);const preventDisabledNavigation=(event:MouseEvent)=>{if(paymentDisabled.value)event.preventDefault();};
const resumableDokuUrl=()=>{const payment=activePayment.value;return paymentProvider.value==='doku'&&hasPendingPayment.value&&payment?.provider==='doku'&&payment.checkout_url&&payment.expired_at&&new Date(payment.expired_at).getTime()>Date.now()?payment.checkout_url:'';};
const onlinePaymentDisabled=computed(()=>!order.value||unsupportedOrderCurrency.value||(order.value.allowed_actions!=null&&!order.value.allowed_actions.includes('continue_payment'))||(paymentProvider.value==='doku'&&hasPendingPayment.value&&!resumableDokuUrl()));
...
const onlineDescription=computed(()=>unsupportedOrderCurrency.value?copy.value.gatewayCurrency:onlinePaymentDisabled.value&&hasPendingPayment.value?copy.value.gatewayPending:copy.value.gatewayContinue.replace('{provider}',paymentProviderLabel.value));
const onlineAction=computed(()=>onlinePaymentDisabled.value&&hasPendingPayment.value?copy.value.paymentPending:submitting.value?copy.value.preparing.replace('{provider}',paymentProviderLabel.value):copy.value.continueTo.replace('{provider}',paymentProviderLabel.value));
...
const loadUsdOrderContext=async()=>{
  ...
  order.value=detail.order;orderItems.value=detail.items||[];
  paymentAttempts.value=detail.payment_attempts||[];
  activePayment.value=detail.latest_payment||null;paymentId.value=detail.latest_payment?.id||'';
  sessionStorage.setItem('iwbif-store-order-id',orderId.value);
  if(paymentId.value)sessionStorage.setItem('iwbif-payment-id',paymentId.value);
  else sessionStorage.removeItem('iwbif-payment-id');
  try{
    const eventId=detail.order.event_id||(await getEvents(1,1)).data[0]?.id;
    if(!eventId)return;
    const catalog=(await getDelegatePackageCatalog(eventId)).data;
    usdPricesByProductId.value=new Map([...catalog.main_packages,...catalog.additional_packages,...(catalog.exhibitor_packages||[])].flatMap(pkg=>pkg.rates).filter(rate=>rate.is_active).map(rate=>[rate.product_id,Number(rate.amount)]));
  }catch{/* The order's IDR total and payment progress remain available. */}
};
const refreshOrder=async()=>{submitting.value=true;errorMessage.value='';try{await loadUsdOrderContext();if(isPaid.value)await refreshPaidFlow();}catch(error){errorMessage.value=apiError(error);}finally{submitting.value=false;}};
...
const startPayment=async()=>{if(!orderId.value||submitting.value||onlinePaymentDisabled.value)return;submitting.value=true;errorMessage.value='';try{const checkout=(await paymentApi.continueOrderPayment(orderId.value,paymentProvider.value)).data;paymentId.value=checkout.payment_id||'';sessionStorage.setItem('iwbif-store-order-id',orderId.value);if(paymentId.value)sessionStorage.setItem('iwbif-payment-id',paymentId.value);const complete=isOrderFullyPaid({status:checkout.order_status,is_payment_complete:checkout.is_payment_complete,remaining_amount:checkout.remaining_amount});if(complete){await refreshPaidFlow();await navigateTo(paidDestination.value);return;}if(checkout.payment_url){window.location.assign(paymentProvider.value==='doku'?validateDokuCheckoutUrl(checkout.payment_url):checkout.payment_url);return;}await navigateTo(`/dashboard/payment-status?order_id=${encodeURIComponent(orderId.value)}&payment_id=${encodeURIComponent(paymentId.value)}`);}catch(error){errorMessage.value=apiError(error);}finally{submitting.value=false;}};
const requestPayment=()=>{if(!orderId.value||submitting.value||onlinePaymentDisabled.value)return;const resumeUrl=resumableDokuUrl();if(resumeUrl){try{window.location.assign(validateDokuCheckoutUrl(resumeUrl));}catch(error){errorMessage.value=apiError(error);}return;}if(showSplitNotice.value){splitModalOpen.value=true;return;}void startPayment();};
const confirmSplitPayment=()=>{splitModalOpen.value=false;void startPayment();};
onMounted(async()=>{
  orderId.value=queryValue(route.query.order_id);
  try{
    if(!orderId.value){await registrationFlow.loadFlow(true);orderId.value=registrationFlow.activeOrderId.value||sessionStorage.getItem('iwbif-store-order-id')||'';}
    if(orderId.value){await loadUsdOrderContext();if(isPaid.value)await refreshPaidFlow();}
