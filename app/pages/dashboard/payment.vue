...
const resumableDokuUrl=()=>{const payment=activePayment.value;return paymentApi.isDokuProvider&&hasPendingPayment.value&&payment?.provider==='doku'&&payment.checkout_url&&payment.expired_at&&new Date(payment.expired_at).getTime()>Date.now()?payment.checkout_url:'';};
const onlinePaymentDisabled=computed(()=>unsupportedOrderCurrency.value||(hasPendingPayment.value&&!resumableDokuUrl()));
...
const onlineDescription=computed(()=>unsupportedOrderCurrency.value?copy.value.gatewayCurrency:hasPendingPayment.value&&!resumableDokuUrl()?copy.value.gatewayPending:copy.value.gatewayContinue.replace('{provider}',paymentProviderLabel.value));
const onlineAction=computed(()=>hasPendingPayment.value&&!resumableDokuUrl()?copy.value.paymentPending:submitting.value?copy.value.preparing.replace('{provider}',paymentProviderLabel.value):copy.value.continueTo.replace('{provider}',paymentProviderLabel.value));
