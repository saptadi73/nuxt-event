<template>
  <section class="mx-auto max-w-5xl px-3 py-10 sm:px-6">
    ...
    <div v-else class="mt-8 grid gap-6 lg:grid-cols-[1fr_22rem]">
      ...
      <aside class="glass-card h-fit rounded-[2rem] p-6"><p class="text-xs uppercase tracking-[.25em] text-slate-400">Order summary</p><div class="mt-5 flex justify-between text-sm text-slate-300"><span>Items</span><span>{{ itemCount }}</span></div><div class="mt-5 flex items-end justify-between border-t border-white/10 pt-5"><span class="text-slate-400">Total</span><strong class="text-2xl text-amber-200">{{ money(total, cart?.currency || items[0]?.currency || items[0]?.product?.currency || 'IDR') }}</strong></div><p v-if="mainPackageRequired" class="mt-4 rounded-xl border border-rose-400/30 bg-rose-500/10 p-3 text-xs text-rose-100">Select one Main Delegate Package (A or B) before checkout.</p><p v-if="invalidPaymentCurrency" class="mt-4 rounded-xl border border-rose-400/30 bg-rose-500/10 p-3 text-xs text-rose-100">Online payment requires all selected packages to use IDR. Please return to the package selection or contact the organizer.</p><button class="mt-6 w-full rounded-full bg-amber-300 px-5 py-3 font-bold text-slate-950 disabled:cursor-not-allowed disabled:opacity-60" :disabled="checkingOut || checkoutBlocked" @click="createOrder">{{ checkingOut ? 'Creating order...' : 'Proceed to checkout' }}</button><p class="mt-3 text-xs leading-5 text-slate-500">The backend validates the required Main package, active rates, currency, and final price.</p></aside>
    </div>
  </section>
</template>
...
const itemCurrencies=computed(()=>[...new Set(items.value.map(item=>(item.currency||item.product?.currency||'').toUpperCase()).filter(Boolean))]);
const invalidPaymentCurrency=computed(()=>{const cartCurrency=(cart.value?.currency||'').toUpperCase();return itemCurrencies.value.length!==1||itemCurrencies.value[0]!=='IDR'||Boolean(cartCurrency&&cartCurrency!=='IDR');});
const checkoutBlocked=computed(()=>mainPackageRequired.value||invalidPaymentCurrency.value);
...
const createOrder=async()=>{if(!eventId.value||checkingOut.value||checkoutBlocked.value)return;checkingOut.value=true;errorMessage.value='';try{const order=(await storeApi.checkout(eventId.value)).data;const orderId=order.order_id||order.id;if(!orderId)throw new Error('The backend did not return an order ID.');sessionStorage.setItem('iwbif-store-order-id',orderId);sessionStorage.setItem('iwbif-store-order',JSON.stringify(order));await navigateTo(`/dashboard/payment?order_id=${encodeURIComponent(orderId)}`);}catch(error){errorMessage.value=apiError(error);}finally{checkingOut.value=false;}};
