    ...
    <div v-else class="mt-8 grid gap-6 lg:grid-cols-[1fr_22rem]">
      <div class="space-y-4">
        <article v-for="item in items" :key="item.product_id" class="glass-card flex flex-col gap-4 rounded-3xl p-5 sm:flex-row sm:items-center sm:justify-between">
          <div><p class="text-xs uppercase tracking-[.2em] text-amber-200">{{ item.product?.product_type || 'Package' }}</p><h2 class="mt-2 text-xl font-bold">{{ item.product?.name || item.product_name || item.name || 'IWBIF Package' }}</h2><p class="mt-2 text-sm text-slate-400">Quantity: {{ item.quantity }}</p></div>
          <div class="sm:text-right"><p class="text-lg font-bold">{{ money(item.subtotal ?? ((item.unit_price || 0) * item.quantity), item.currency || item.product?.currency || cart?.currency || 'IDR') }}</p><button class="mt-3 text-sm font-semibold text-red-300 disabled:opacity-50" :disabled="removingId === item.product_id" @click="remove(item.product_id)">{{ removingId === item.product_id ? 'Removing...' : 'Remove' }}</button></div>
        </article>
      </div>
      <aside class="glass-card h-fit rounded-[2rem] p-6"><p class="text-xs uppercase tracking-[.25em] text-slate-400">Order summary</p><div class="mt-5 flex justify-between text-sm text-slate-300"><span>Items</span><span>{{ itemCount }}</span></div><div class="mt-5 flex items-end justify-between border-t border-white/10 pt-5"><span class="text-slate-400">Total</span><strong class="text-2xl text-amber-200">{{ money(total, cart?.currency || items[0]?.currency || items[0]?.product?.currency || 'IDR') }}</strong></div><button class="mt-6 w-full rounded-full bg-amber-300 px-5 py-3 font-bold text-slate-950 disabled:cursor-not-allowed disabled:opacity-60" :disabled="checkingOut" @click="createOrder">{{ checkingOut ? 'Creating order...' : 'Proceed to checkout' }}</button><p class="mt-3 text-xs leading-5 text-slate-500">The backend validates availability, quantity, currency, and final price.</p></aside>
    </div>
  </section>
</template>
...
const createOrder=async()=>{if(!eventId.value||checkingOut.value)return;checkingOut.value=true;errorMessage.value='';try{const cartSnapshot=(await storeApi.getCart(eventId.value)).data;const delegatePackageId=Array.isArray(cartSnapshot.items)?cartSnapshot.items.find(item=>item.product?.product_type==='delegate')?.product?.metadata_json?.delegate_package_id:undefined;const order=(await storeApi.checkout(eventId.value)).data;const orderId=order.order_id||order.id;if(!orderId)throw new Error('The backend did not return an order ID.');sessionStorage.setItem('iwbif-store-order-id',orderId);sessionStorage.setItem('iwbif-store-order',JSON.stringify(order));if(typeof delegatePackageId==='string'&&delegatePackageId){sessionStorage.setItem('iwbif-last-delegate-package-id',delegatePackageId);}await navigateTo(`/dashboard/payment?order_id=${encodeURIComponent(orderId)}`);}catch(error){errorMessage.value=apiError(error);}finally{checkingOut.value=false;}};
