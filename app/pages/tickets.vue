      ...
      <section class="mt-9">
        ...
        <div class="mt-5 grid gap-5 lg:grid-cols-2">
          <article v-for="pkg in catalog.main_packages" :key="pkg.id" class="rounded-3xl border p-5 transition sm:p-7" :class="selection.mainPackageId === pkg.id ? 'border-amber-300 bg-amber-300/10' : 'border-white/10 bg-white/5'">
            <label class="flex cursor-pointer items-start gap-3"><input type="radio" name="main-package" :checked="selection.mainPackageId === pkg.id" class="mt-1 h-5 w-5 shrink-0 accent-amber-300" @change="selectMainPackage(pkg)"><span class="min-w-0"><span class="text-xs font-bold uppercase tracking-[.25em] text-amber-200">{{ copy.package }} {{ pkg.code }}</span><span class="mt-1 block break-words text-2xl font-bold">{{ pkg.name }}</span><span v-if="pkg.description" class="mt-2 block break-words text-sm text-slate-400">{{ pkg.description }}</span></span></label>
...
const messages={
  en:{delegatePackages:'Delegate Packages',exhibitorPackages:'Exhibitor Packages',title:'Choose your IWBIF experience.',intro:'Choose exactly one Main package. Sharing is selected by default; Single uses its own final tariff. The Bandung trip is optional.',required:'Required',selectMain:'1. Select Main Package',selectionRequired:'Selection required',optional:'Optional add-on',additionalTrip:'2. Additional Trip',addTrip:'Add this trip to your Main package.',updating:'Updating cart…',review:'Review cart',exhibitorUnavailable:'Exhibitor package purchasing is not available yet.',selectMessage:'Select Package A or B to continue.',paymentMissing:'The selected rate does not yet have an IDR payment amount. Please contact the organizer.',selected:'Main package selected. Review the official total in your cart.',single:'Single Occupancy',sharing:'Sharing Occupancy',updateError:'Package selection could not be updated.',saved:'Your package selection has been saved.',removed:'Additional trip removed.',noEvent:'No event is currently published.',package:'Package',jakartaItinerary:'View Jakarta itinerary',bandungItinerary:'View Bandung itinerary',idrUnavailable:'IDR payment unavailable',paidAs:'Paid as {amount}',closeItinerary:'Close itinerary',highlights:'Event highlights',notes:'Important notes'},
  'zh-CN':{delegatePackages:'代表套餐',exhibitorPackages:'参展商套餐',title:'选择您的 IWBIF 体验。',intro:'请仅选择一个主套餐。默认选择合住；单人住宿适用其独立最终价格。万隆行程为可选项。',required:'必选',selectMain:'1. 选择主套餐',selectionRequired:'必须选择',optional:'可选附加项',additionalTrip:'2. 附加行程',addTrip:'将此行程添加到主套餐。',updating:'正在更新购物车…',review:'查看购物车',exhibitorUnavailable:'参展商套餐暂不可购买。',selectMessage:'请选择套餐 A 或 B 以继续。',paymentMissing:'所选价格尚未配置印尼盾付款金额。请联系主办方。',selected:'已选择主套餐。请在购物车中查看官方总额。',single:'单人住宿',sharing:'合住',updateError:'无法更新套餐选择。',saved:'您的套餐选择已保存。',removed:'附加行程已移除。',noEvent:'当前尚未发布活动。',package:'套餐',jakartaItinerary:'查看雅加达行程',bandungItinerary:'查看万隆行程',idrUnavailable:'暂不支持印尼盾付款',paidAs:'以 {amount} 支付',closeItinerary:'关闭行程',highlights:'活动亮点',notes:'重要说明'}
} as const;
