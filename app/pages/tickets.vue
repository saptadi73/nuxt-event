<template>
  <section class="mx-auto max-w-7xl px-3 py-10 sm:px-6 lg:px-8">
    ...
    <template v-else-if="selectedType === 'delegate'">
      <section class="mt-9">
        <div class="flex items-end justify-between gap-3"><div><p class="text-xs uppercase tracking-[.28em] text-amber-200">Required</p><h2 class="mt-2 text-2xl font-black">1. Select Main Package</h2></div><span v-if="!selection.mainProductId" class="text-xs font-semibold text-rose-200">Selection required</span></div>
        <div class="mt-5 grid gap-5 lg:grid-cols-2">
          <article v-for="pkg in catalog.main_packages" :key="pkg.id" class="rounded-3xl border p-5 transition sm:p-7" :class="selection.mainPackageId === pkg.id ? 'border-amber-300 bg-amber-300/10' : 'border-white/10 bg-white/5'">
            <label class="flex cursor-pointer items-start gap-3"><input type="radio" name="main-package" :checked="selection.mainPackageId === pkg.id" class="mt-1 h-5 w-5 accent-amber-300" @change="selectMainPackage(pkg)"><span><span class="text-xs font-bold uppercase tracking-[.25em] text-amber-200">Package {{ pkg.code }}</span><span class="mt-1 block text-2xl font-bold">{{ pkg.name }}</span><span v-if="pkg.description" class="mt-2 block text-sm text-slate-400">{{ pkg.description }}</span></span></label>
      ...
      <section v-if="catalog.additional_packages.length" class="mt-9">
        ...
        <article v-for="pkg in catalog.additional_packages" :key="pkg.id" class="mt-5 rounded-3xl border p-5 sm:p-7" :class="selection.bandungSelected ? 'border-cyan-300/60 bg-cyan-300/10' : 'border-white/10 bg-white/5'">
          <label class="flex cursor-pointer items-start gap-3"><input type="checkbox" :checked="selection.bandungSelected" :disabled="mutating" class="mt-1 h-5 w-5 accent-cyan-300" @change="toggleAdditional(pkg, ($event.target as HTMLInputElement).checked)"><span><span class="text-xl font-bold">{{ pkg.name }}</span><span class="mt-1 block text-sm text-slate-400">Add this trip to your Main package.</span></span></label>
