<template>
  <section class="mx-auto max-w-7xl px-3 py-10 sm:px-6 lg:px-8">
    ...
    <NuxtLink to="/register" class="mt-6 inline-flex rounded-full border border-white/15 px-5 py-3 text-sm font-bold text-slate-300 transition hover:bg-white/5">{{ copy.participationType }}</NuxtLink>
    ...
    <template v-else-if="selectedType === 'delegate'">
      ...
      <aside class="mt-9 rounded-2xl border border-cyan-200/20 bg-cyan-400/5 p-5">
        <p class="text-sm leading-7 text-slate-300">{{ copy.exhibitorAlongside }}</p>
        <NuxtLink to="/register" class="mt-3 inline-flex text-sm font-bold text-cyan-200 underline underline-offset-4">{{ copy.participationType }}</NuxtLink>
      </aside>
...
useSeoMeta({title:()=>`${selectedType.value === 'exhibitor' ? copy.value.exhibitorPackages : copy.value.delegatePackages} | IWBIF 2026`,description:()=>copy.value.intro});
