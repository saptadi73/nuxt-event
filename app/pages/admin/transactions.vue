<template>
  <section class="mx-auto max-w-7xl px-3 py-10 sm:px-6 lg:px-8">
    ...
    <Teleport to="body"><div v-if="dialog.open" class="backdrop" @click.self="closeDialog"><form class="modal max-w-xl" @submit.prevent="submitAction"><header><div><p class="text-xs uppercase tracking-[.24em] text-amber-200">Transaction action</p><h2>{{ actionTitle }}</h2></div><button type="button" @click="closeDialog">×</button></header><main><p class="rounded-2xl border border-amber-300/20 bg-amber-300/5 p-4 text-sm text-amber-100">This will {{ dialog.action }} {{ dialog.ids.length }} transaction(s). The backend will validate the action atomically.</p><label class="field mt-5"><span>Verification notes</span><textarea v-model.trim="dialog.notes" rows="4" maxlength="1000" required placeholder="Explain the verification or reason for this action" /></label><label v-if="dialog.action === 'success'" class="field mt-4"><span>Paid at <small>(optional)</small></span><input v-model="dialog.paid_at" type="datetime-local"></label></main><footer><button type="button" class="action-secondary" :disabled="saving" @click="closeDialog">Back</button><button class="action-primary" :disabled="saving || !dialog.notes">{{ saving ? 'Processing...' : actionTitle }}</button></footer></form></div></Teleport>
  </section>
</template>
...
const dialog = reactive<{ open: boolean; action: TransactionAction; ids: string[]; notes: string; paid_at: string }>({ open: false, action: 'success', ids: [], notes: '', paid_at: '' });
