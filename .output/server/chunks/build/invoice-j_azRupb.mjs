globalThis.__timing__.logStart('Load chunks/build/invoice-j_azRupb');import { a as useSeoMeta$1, N as NuxtLink } from '../virtual/entry.mjs';
...
var invoice_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-4xl px-4 py-12 sm:px-6" }, _attrs))}><p class="text-sm uppercase tracking-[.35em] text-cyan-200">Payment and Invoice</p><h1 class="mt-3 text-4xl font-black">Registration invoice</h1>`);
			if (unref(pending)) _push(`<div class="glass-card mt-8 rounded-[2rem] p-7 text-slate-300">Loading invoice...</div>`);
				...
				_push(`<div class="glass-card mt-8 rounded-[2rem] p-7"><p class="text-lg font-semibold">Belum ada invoice yang tersedia.</p><p class="mt-2 text-slate-400">Invoice akan tersedia setelah pembayaran berhasil dikonfirmasi.</p>`);
				_push(ssrRenderComponent(_component_NuxtLink, {
					to: "/dashboard/payment",
					class: "mt-6 inline-flex rounded-full bg-cyan-400 px-5 py-3 font-semibold text-slate-950"
				}, {
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`Ke pembayaran`);
						else return [createTextVNode("Ke pembayaran")];
					}),
					_: 1
				}, _parent));
				_push(`</div>`);
			} else _push(`<article id="invoice" class="glass-card mt-8 rounded-[2rem] p-7"><div class="flex flex-wrap justify-between gap-5 border-b border-white/10 pb-6"><div><p class="text-sm text-slate-400">${ssrInterpolate(unref(invoice).registration.event_name)}</p><p class="mt-1 font-semibold">Invoice ${ssrInterpolate(unref(invoice).order.order_number)}</p></div><span class="h-fit rounded-full bg-emerald-300/10 px-4 py-2 text-xs font-semibold uppercase tracking-[.2em] text-emerald-200">Paid</span></div><dl class="mt-6 grid gap-5 sm:grid-cols-2"><div><dt class="text-xs uppercase tracking-[.2em] text-slate-500">Registration number</dt><dd class="mt-2 text-lg font-semibold">${ssrInterpolate(unref(invoice).registration.registration_number)}</dd></div><div><dt class="text-xs uppercase tracking-[.2em] text-slate-500">Participant</dt><dd class="mt-2 text-lg font-semibold">${ssrInterpolate(unref(invoice).participant.full_name)}</dd><p class="text-sm text-slate-400">${ssrInterpolate(unref(invoice).participant.email)}</p></div><div><dt class="text-xs uppercase tracking-[.2em] text-slate-500">Ticket category</dt><dd class="mt-2 text-lg font-semibold">${ssrInterpolate(unref(invoice).registration.ticket_type_name || "-")}</dd></div><div><dt class="text-xs uppercase tracking-[.2em] text-slate-500">Payment status</dt><dd class="mt-2 text-lg font-semibold text-emerald-300">Lunas</dd><p class="text-sm text-slate-400">${ssrInterpolate(formatDate(unref(invoice).payment.paid_at))}</p></div></dl><div class="mt-7 flex items-center justify-between border-t border-white/10 pt-6"><span class="text-slate-400">Total paid</span><strong class="text-2xl text-cyan-200">${ssrInterpolate(formatCurrency(unref(invoice).order.total_amount, unref(invoice).order.currency))}</strong></div><button class="mt-7 rounded-full bg-cyan-400 px-5 py-3 font-semibold text-slate-950 print:hidden">Download invoice</button></article>`);
			_push(`</section>`);
		};
	}
});
...
export { invoice_default as default };;globalThis.__timing__.logEnd('Load chunks/build/invoice-j_azRupb');
//# sourceMappingURL=invoice-j_azRupb.mjs.map
