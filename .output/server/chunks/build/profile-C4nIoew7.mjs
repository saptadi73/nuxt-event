globalThis.__timing__.logStart('Load chunks/build/profile-C4nIoew7');import { u as useMediaUrl } from './useMediaUrl-CtPklTwb.mjs';
...
var profile_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8" }, _attrs))}><h1 class="text-4xl font-black text-white">Profil Saya</h1><p class="mt-3 text-slate-300">Update data yang digunakan endpoint \`PUT /participants/me\` dan \`PATCH /participants/me\`.</p><div class="mt-8 grid gap-4 rounded-[2rem] border border-white/10 bg-white/5 p-6"><div class="flex flex-wrap items-center gap-4">`);
			...
			_push(`<label class="grid gap-2 text-sm text-slate-300"><span>Profile photo</span><input type="file" accept="image/jpeg,image/png,image/webp" class="block text-sm text-slate-300 file:mr-3 file:rounded-full file:border-0 file:bg-cyan-400 file:px-4 file:py-2 file:font-semibold file:text-slate-950"></label></div><label class="grid gap-2"><span class="text-sm text-slate-300">Nama Lengkap</span><input${ssrRenderAttr("value", unref(form).full_name)} class="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none"></label><label class="grid gap-2"><span class="text-sm text-slate-300">Organisasi</span><input${ssrRenderAttr("value", unref(form).organization_name)} class="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none"></label><label class="grid gap-2"><span class="text-sm text-slate-300">Biografi</span><textarea rows="5" class="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-white outline-none">${ssrInterpolate(unref(form).biography)}</textarea></label><div class="flex flex-wrap gap-3"><button class="rounded-full bg-cyan-400 px-6 py-3 text-sm font-semibold text-slate-950"> Simpan Semua </button><button class="rounded-full border border-white/15 px-6 py-3 text-sm font-semibold text-white"> Simpan Parsial </button></div></div>`);
...
export { profile_default as default };;globalThis.__timing__.logEnd('Load chunks/build/profile-C4nIoew7');
//# sourceMappingURL=profile-C4nIoew7.mjs.map
