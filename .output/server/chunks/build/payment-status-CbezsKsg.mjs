globalThis.__timing__.logStart('Load chunks/build/payment-status-CbezsKsg');import { u as usePayment } from './usePayment-s4-2mR-w.mjs';
...
var payment_status_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8" }, _attrs))}><div class="glass-card rounded-[2rem] p-6"><p class="text-sm uppercase tracking-[0.3em] text-cyan-200/70">Payment Status</p><h1 class="mt-3 text-4xl font-bold text-white">Monitor payment status</h1><p class="mt-3 text-slate-300"> Check the latest status from \`GET /orders/{order_id}\` and \`GET /payments/{payment_id}\`. </p><div class="mt-8 grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end"><label class="grid gap-2"><span class="text-sm text-slate-300">Order ID</span><input${ssrRenderAttr("value", unref(orderId))} class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" placeholder="uuid"></label><label class="grid gap-2"><span class="text-sm text-slate-300">Payment ID</span><input${ssrRenderAttr("value", unref(paymentId))} class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none" placeholder="uuid"></label><button class="rounded-full bg-cyan-400 px-6 py-3 text-sm font-semibold text-slate-950"${ssrIncludeBooleanAttr(unref(checking)) ? " disabled" : ""}>${ssrInterpolate(unref(checking) ? "Loading..." : "Check Status")}</button></div><div class="mt-6 grid gap-4 md:grid-cols-2"><article class="rounded-[1.5rem] border border-white/10 bg-white/5 p-4"><p class="text-xs uppercase tracking-[0.25em] text-slate-400">Order</p>`);
			...
			else _push(`<p class="mt-2 text-sm text-slate-300">No order queried yet.</p>`);
			...
			else _push(`<p class="mt-2 text-sm text-slate-300">No payment queried yet.</p>`);
			_push(`</article></div>`);
...
export { payment_status_default as default };;globalThis.__timing__.logEnd('Load chunks/build/payment-status-CbezsKsg');
//# sourceMappingURL=payment-status-CbezsKsg.mjs.map
