globalThis.__timing__.logStart('Load chunks/build/workshops-ltmuT-wW');import { a as useSeoMeta$1, b as useAsyncData } from '../virtual/entry.mjs';
...
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8" }, _attrs))}><p class="text-sm uppercase tracking-[.35em] text-amber-200">Delegate Activities</p><h1 class="mt-4 max-w-4xl text-5xl font-black">Build your IWBIF experience.</h1><p class="mt-5 max-w-3xl text-lg leading-8 text-slate-300">Select from the forum, business matching, exhibition, networking, and industrial visit activities published by the organizer.</p>`);
			if (unref(pending)) {
				...
				ssrRenderList(unref(activities), (item) => {
					_push(`<article class="glass-card rounded-[2rem] p-7"><h2 class="text-2xl font-bold">${ssrInterpolate(item.name)}</h2>`);
					if (item.description) _push(`<p class="mt-3 leading-7 text-slate-300">${ssrInterpolate(item.description)}</p>`);
					else _push(`<!---->`);
					_push(`</article>`);
				});
				_push(`<!--]--></div>`);
...
export { workshops_default as default };;globalThis.__timing__.logEnd('Load chunks/build/workshops-ltmuT-wW');
//# sourceMappingURL=workshops-ltmuT-wW.mjs.map
