...
var announcements_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-4xl px-3 py-10 sm:px-6" }, _attrs))}><p class="text-sm uppercase tracking-[.35em] text-cyan-200">Event Announcements</p><h1 class="mt-3 text-3xl font-black sm:text-4xl">Important participant updates</h1><div class="mt-8 space-y-5"><!--[-->`);
			ssrRenderList(items, (item) => {
				_push(`<article class="glass-card rounded-3xl p-4 sm:p-6"><div class="flex flex-wrap justify-between gap-3"><span class="text-xs uppercase tracking-[.25em] text-orange-200">${ssrInterpolate(item.type)}</span><time class="text-xs text-slate-500">${ssrInterpolate(item.date)}</time></div><h2 class="mt-3 text-lg font-bold sm:text-xl">${ssrInterpolate(item.title)}</h2><p class="mt-3 text-sm leading-7 text-slate-300 sm:text-base">${ssrInterpolate(item.text)}</p></article>`);
			});
			_push(`<!--]--></div></section>`);
		};
	}
