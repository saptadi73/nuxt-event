globalThis.__timing__.logStart('Load chunks/build/partners-BHwd5Qkc');import { a as useSeoMeta$1 } from '../virtual/entry.mjs';
...
var partners_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8" }, _attrs))}><p class="text-sm uppercase tracking-[0.35em] text-cyan-200">Partners and Sponsors</p><h1 class="mt-4 max-w-4xl text-5xl font-black">Connect global partners with women-led business opportunities</h1><p class="mt-5 max-w-3xl text-lg leading-8 text-slate-300">Build trust-based visibility with ecosystem brands, financial institutions, chambers, and international collaborators.</p><div class="mt-10 grid gap-5 md:grid-cols-2"><!--[-->`);
			...
			_push(`<!--]--></div><div class="mt-8 grid gap-6 overflow-hidden rounded-[2rem] bg-white/5 p-7 md:grid-cols-2"><img${ssrRenderAttr("src", _virtual_public__2Ffoto_2Fpameran_default)} alt="Partner exhibition at IWBIF" class="h-64 w-full rounded-3xl object-cover"><div class="self-center"><h2 class="text-3xl font-bold">Support the Future of International Women Entrepreneurship</h2><p class="mt-3 leading-7 text-slate-300">Help businesses build sustainable cross-border relationships through sponsorship, strategic sessions, and program co-creation.</p><a href="mailto:partners@iwbif-event.example" class="mt-6 inline-flex rounded-full bg-cyan-400 px-6 py-3 font-semibold text-slate-950">Contact Partnership Team</a></div></div></section>`);
		};
	}
});
//#endregion
...
export { partners_default as default };;globalThis.__timing__.logEnd('Load chunks/build/partners-BHwd5Qkc');
//# sourceMappingURL=partners-BHwd5Qkc.mjs.map
