globalThis.__timing__.logStart('Load chunks/build/dashboard-BV-Id95C');import { a as useSeoMeta$1, N as NuxtLink } from '../virtual/entry.mjs';
...
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		useSeoMeta$1({ title: "Participant Dashboard | ASEAN AI for Education" });
		const days = Math.max(0, Math.ceil(((/* @__PURE__ */ new Date("2026-11-18T07:30:00+07:00")).getTime() - Date.now()) / 864e5));
		...
		const menu = [
			...
			},
			{
				...
				text: "Discover potential collaborators across ASEAN."
			},
			{
				to: "/dashboard/schedule",
				label: "Agenda",
				title: "My Schedule",
				text: "Review the two-day event program."
			},
			{
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8" }, _attrs))}><p class="text-sm uppercase tracking-[.35em] text-cyan-200">Participant Dashboard</p><div class="mt-3 flex flex-wrap items-end justify-between gap-5"><div><h1 class="text-4xl font-black">Welcome to ASEAN AI for Education Summit 2026</h1><p class="mt-3 text-slate-300">Your ticket, schedule, profile, networking, payment, and event updates in one place.</p></div><div class="glass-card rounded-2xl px-5 py-3 text-right"><p class="text-xs uppercase tracking-[.25em] text-slate-400">Event starts in</p><p class="mt-1 text-xl font-bold text-cyan-200">${ssrInterpolate(unref(countdown))}</p></div></div><div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><!--[-->`);
...
export { dashboard_default as default };;globalThis.__timing__.logEnd('Load chunks/build/dashboard-BV-Id95C');
//# sourceMappingURL=dashboard-BV-Id95C.mjs.map
