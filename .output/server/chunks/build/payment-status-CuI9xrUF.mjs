globalThis.__timing__.logStart('Load chunks/build/payment-status-CuI9xrUF');import { a as useSeoMeta$1, f as useRoute, N as NuxtLink } from '../virtual/entry.mjs';
...
var payment_status_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const requestId = ref("");
		const terminal = computed(() => ["failed", "expired"].includes(status.value));
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-3xl px-4 py-10 sm:px-6" }, _attrs))}><div class="glass-card rounded-[2rem] p-7"><p class="text-sm uppercase tracking-[.3em] text-amber-200">DOKU Payment Status</p><h1 class="mt-3 text-4xl font-bold">${ssrInterpolate(unref(heading))}</h1><p class="mt-3 text-slate-300">${ssrInterpolate(unref(description))}</p><div class="mt-7 rounded-2xl border border-white/10 bg-white/5 p-5"><p class="text-xs uppercase tracking-widest text-slate-400">Payment status</p><p class="${ssrRenderClass([unref(statusClass), "mt-2 text-2xl font-bold"])}">${ssrInterpolate(unref(statusLabel))}</p>`);
			...
			_push(`</div><div class="mt-6 flex flex-wrap gap-3">`);
...
export { payment_status_default as default };;globalThis.__timing__.logEnd('Load chunks/build/payment-status-CuI9xrUF');
//# sourceMappingURL=payment-status-CuI9xrUF.mjs.map
